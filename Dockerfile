...
ARG DOCKER_BUILDPACK=3.4.0
ARG CF_BUILDPACK_URL=https://github.com/xiwenc/cf-mendix-buildpack/archive/refs/heads/prometheus.zip
#ARG CF_BUILDPACK_URL=https://github.com/mendix/cf-mendix-buildpack/releases/download/v4.21.0/cf-mendix-buildpack.zip
ARG DOCKER_BUILDPACK_URL=https://github.com/mendix/docker-mendix-buildpack/archive/refs/tags/v${DOCKER_BUILDPACK}.tar.gz
...
#ARG BLOBSTORE=http://192.168.201.175:8000
#ARG BLOBSTORE=http://192.168.43.83:8000
...
RUN mkdir -p /opt/mendix/buildpack /opt/mendix/build &&\
    ...
    mv /opt/mendix/buildpack/cf-mendix-buildpack-prometheus/* /opt/mendix/buildpack/ &&\
    chown -R ${USER_UID}:0 /opt/mendix &&\
    chmod -R g=u /opt/mendix 

RUN echo "Downloading Docker Buildpack from ${DOCKER_BUILDPACK_URL}" &&\
    mkdir /tmp/scripts && \
    cd /tmp/scripts && \
    curl -fsSL ${DOCKER_BUILDPACK_URL} | tar -zxv --strip-components=2 docker-mendix-buildpack-${DOCKER_BUILDPACK}/scripts

# Copy python scripts which execute the buildpack (exporting the VCAP variables)
RUN cp /tmp/scripts/compilation /tmp/scripts/git /opt/mendix/buildpack/
...
COPY --from=builder /tmp/scripts/startup /opt/mendix/build/
COPY --from=builder /tmp/scripts/vcap_application.json /opt/mendix/build/
